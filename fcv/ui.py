...
# ---------------------------------------------------------------- utilitas
def inject_css() -> None:
    # CSS-only HTML goes into Streamlit's event container, not the reading order.
    st.html(CSS_FILE)
...
def section(title: str, note: str = "") -> None:
    st.markdown(
        f'<div class="fc-sec"><span class="fc-sec__bar" aria-hidden="true"></span>'
        f'<h2 class="fc-sec__title">{escape(title)}</h2>'
...
def app_header(dq, model_note: str) -> None:
    st.markdown(
        '<header class="fc-top"><div class="fc-top__identity">'
        '<div class="fc-top__mark" aria-hidden="true">FCV</div><div>'
        '<h1 class="fc-top__title">Football Valuation Centre</h1>'
        '<div class="fc-top__sub">Kenali profil. Bandingkan nilai. Temukan kandidat.</div></div></div>'
        '<div class="fc-top__stats" role="group" aria-label="Ringkasan dataset">'
        ...
        f'<div class="fc-top__pill">Median error <b>{escape(model_note)}</b></div>'
        "</div></header>",
        unsafe_allow_html=True,
    )


def filter_result(matches: int, total: int) -> str:
    state = " is-empty" if matches == 0 else ""
    return (f'<div class="fc-filter-result{state}" role="status" aria-live="polite">'
            f'<b>{count(matches)} pemain cocok</b>'
            f'<span>dari {count(total)} pemain · satu snapshot per pemain</span></div>')
...
def grid(cards: list[str], columns: int = 3) -> str:
    ...
    bertangga.
    """
    return (f'<div class="fc-grid fc-grid--{columns}">'
            f'{"".join(cards)}</div>')
...
def player_card(row: pd.Series, show_ribbon: bool = True) -> str:
    ...
    return (
        ...
        f'<div class="fc-card__ovr">{int(row["ovr"])}</div><div class="fc-card__ovr-label">OVR est.</div>'
...
def percentile_bars(row: pd.Series, cols: list[str] | None = None) -> str:
    ...
    for col in cols:
        ...
        rows.append(
            ...
            f'<span class="fc-bar__track" role="img" aria-label="{CARD_STAT_LABEL.get(col, col)}: persentil {p:.0f} dari 100">'
            f'<span class="fc-bar__fill {cls}" '
            f'style="width:{p:.0f}%"></span></span>'
            f'<span class="fc-bar__v">{raw}<small> · P{p:.0f}</small></span></div>'
        )
...
def mini_card(row: pd.Series, note: str | None = None) -> str:
    ...
    return (
        ...
        f'<div class="fc-mini__val"><div class="fc-mini__price-label">Harga pasar</div>'
        f'<div class="now">{escape(money(row["value_eur"]))}</div>'
...
def _dark(fig: go.Figure, height: int = 340, legend: bool = False) -> go.Figure:
    ...
    fig.update_layout(
        ...
        legend=dict(orientation="h", yanchor="bottom", y=1.06, x=0,
                    ...
                    font=dict(family=BODY_FONT, size=12, color=DIM)),
    ...
    axis = dict(gridcolor="rgba(255,255,255,.06)", zerolinecolor="rgba(255,255,255,.12)",
                ...
                tickfont=dict(family=BODY_FONT, size=12, color=DIM),
                title_font=dict(family=BODY_FONT, size=12, color=DIM_2),
                title_standoff=12)
...
def market_scatter(df: pd.DataFrame, highlight: pd.Series | None = None) -> go.Figure:
    ...
    for verdict_name, colour in colours.items():
        ...
        fig.add_trace(go.Scattergl(
            ...
            hovertemplate="%{text}<br>Pasar €%{x:.1f} jt · Model €%{y:.1f} jt<extra></extra>",
        ))
...
def compare_bars(a: pd.Series, b: pd.Series) -> go.Figure:
    labels = [CARD_STAT_LABEL[s] for s in CARD_STATS]
    fig = go.Figure()
    fig.add_trace(go.Bar(x=labels, y=[a[s] for s in CARD_STATS], name=f"A · {a['short_name']}",
                         marker_color=GREEN, opacity=.9))
    fig.add_trace(go.Bar(x=labels, y=[b[s] for s in CARD_STATS], name=f"B · {b['short_name']}",
                         marker_color=CYAN, marker_pattern_shape="/", opacity=.9))
